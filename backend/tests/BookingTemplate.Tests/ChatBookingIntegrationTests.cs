...
public sealed class ChatBookingIntegrationTests
{
    ...
    private sealed class FakeGeminiChatService(BookingChatToolExecutor toolExecutor) : IGeminiChatService
    {
        public async Task<ChatResponseDto?> ReplyWithGeminiAndToolsAsync(string message, string? sessionId, CancellationToken cancellationToken)
        {
            ...
            {
                var details = new PendingBooking(
                    "Full Groom",
                ...
                var token = Convert.ToBase64String(JsonSerializer.SerializeToUtf8Bytes(details));
                var prompt = $"Please confirm your booking details:\n" +
                             $"Service: {details.ServiceName}\n" +
                             $"Date: {details.Date}\n" +
                             $"Time: {details.StartTime}\n" +
                             $"Pet: {details.PetName} ({details.PetType})\n" +
                             $"Customer: {details.CustomerName}\n" +
                             $"Phone: {details.Phone}\n\n" +
                             $"If everything is correct, reply exactly:\nYes, confirm {token}";
                return new ChatResponseDto(prompt, "booking_confirmation");
            }
        ...
        private static bool TryParseConfirmation(string message, out PendingBooking pending)
        {
            ...
            try
            {
                ...
                var parsed = JsonSerializer.Deserialize<PendingBooking>(bytes);
                if (parsed is null)
                {
                    return false;
                }
